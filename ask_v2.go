...
func cmdAskV2(ctx context.Context, query, alt, altLang, scope string, profile CorpusProfile, start time.Time) error {
	response, err := searchViaAPIV2(ctx, query, alt, altLang, scope, start)
	if err != nil {
	...
	prompt := buildAskPrompt(response, contextText, query, indexText)
...
func buildAskPrompt(response v2SearchResponse, contextText, query, indexText string) string {
	return fmt.Sprintf(`You are an assistant that answers solely based on content from personal knowledge-base corpora.
...
- First decide whether the supplied excerpts directly contain the specific fact, setting, procedure, or conclusion requested. Topic overlap alone is not an answer.
- Do not infer missing steps or settings from adjacent behavior, a default value, an example, or plausible outside knowledge.
- If the context is only partially relevant and lacks the requested detail, explicitly say which detail the KB does not contain. You may report the supported partial facts, clearly labelled as partial, but must not extrapolate them into an answer.
...
func searchViaAPIV2(ctx context.Context, query, alt, altLang, scope string, start time.Time) (v2SearchResponse, error) {
	...
	if alt != "" {
		payload["query_alt"] = alt
		payload["query_alt_language"] = altLang
	}
